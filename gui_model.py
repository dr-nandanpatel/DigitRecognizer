from load_model import testing
...
def clear():
    cv.delete('all')
    draw.rectangle((0, 0, 600, 600), fill=(255, 255, 255, 0))
    txt.delete('1.0', END)
...
btnModel = Button(text="Predict Digit", command=model)
btnClear = Button(text="Clear Canvas", command=clear)
##button.pack()
btnModel.pack()
btnClear.pack()
txt.pack()
root.title('Digit Recognizer -- GUI')
root.mainloop()